...
export class AuthenticationService {
    ...
    login(login: string, password: string) {
        return this._http.post<any>(`${BACKEND_SETTINGS.apiUrl}/users/authenticate`, { login, password }, { withCredentials: true })
            .pipe(map(user => {
    ...
    logout() {
        this._http.post<any>(`${BACKEND_SETTINGS.apiUrl}/users/token/revoke`, {}, { withCredentials: true }).subscribe();
    ...
    refreshToken() {
        return this._http.post<any>(`${BACKEND_SETTINGS.apiUrl}/users/token/refresh`, {}, { withCredentials: true })
            .pipe(map((user) => {
                this._userSubject.next(user);
                this.startRefreshTokenTimer();
                return user;
