...
namespace DocumentSql.Services
{
    public class DefaultQuery : IQuery
    {
        ...
        public static Dictionary<MethodInfo, Action<DefaultQuery, StringBuilder, ISqlDialect, MethodCallExpression>> MethodMappings = new Dictionary<MethodInfo, Action<DefaultQuery, StringBuilder, ISqlDialect, MethodCallExpression>>();
        ...
        private ConstantExpression Evaluate(Expression expression)
        {
            switch (expression.NodeType)
            {
                ...
                case ExpressionType.New:
                    ...
                    var arguments = newExpression.Arguments.Select(f => Evaluate(f).Value).ToArray();
                    var value = newExpression.Constructor.Invoke(arguments);
                    return Expression.Constant(value);

                case ExpressionType.Call:
                    var methodExpression = (MethodCallExpression)expression;
                    arguments = methodExpression.Arguments.Select(f => Evaluate(f).Value).ToArray();
                    object obj = null;
        ...
        IQuery<T> IQuery.For<T>(bool filterType)
        {
            ...
            if (filterType)
            {
                _queryState._sqlBuilder.WhereAlso(_queryState._sqlBuilder.FormatColumn(_queryState._documentTable, "Type") + " = " + _dialect.QuoteForParameter("Type"));
        ...
        public class Query<T> : IQuery<T> where T : class
        {
            ...
            {
                // TODO: [IAsyncEnumerable] Заменить это когда добавится поддержка в Dapper
            ...
            public async Task<IEnumerable<T>> ListImpl()
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (!_query._dialect.IsSpecialDistinctRequired)
                        {
        ...
        class QueryIndex<T> : Query<T>, IQueryIndex<T> where T : class, IIndex
        {
            ...
            {
                // TODO: [IAsyncEnumerable] Заменить это после добавления поддержки в Dapper
                foreach (var item in await ListImpl())
                {
                    yield return item;
                }
