...
namespace DocumentSql.Sql
{
    public abstract class BaseCommandInterpreter : ICommandInterpreter
    {
        ...
        public virtual IEnumerable<string> Run(ICreateTableCommand command)
        {
            ...
            builder
                .Append(_dialect.CreateTableString)
                .Append(' ')
            ...
            var primaryKeys = command.TableCommands.OfType<CreateColumnCommand>().Where(f => f.IsPrimaryKey && !f.IsIdentity).Select(f => _dialect.QuoteForColumnName(f.ColumnName)).ToArray();
            if (primaryKeys.Any())
            {
                if (appendComma)
                {
                ...
                builder
                    .Append(_dialect.PrimaryKeyString)
                    .Append(" ( ")
